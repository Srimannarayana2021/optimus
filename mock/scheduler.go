...
func (ms *Scheduler) GetDagRunStatus(ctx context.Context, jobSpec models.JobSpec, startDate time.Time,
	endDate time.Time, batchSize int) ([]models.JobStatus, error) {
	args := ms.Called(ctx, jobSpec, startDate, endDate, batchSize)
	return args.Get(0).([]models.JobStatus), args.Error(1)
}
