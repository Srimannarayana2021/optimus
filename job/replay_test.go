...
func TestReplay(t *testing.T) {
	...
	specs[spec1] = models.JobSpec{Name: spec1, Dependencies: noDependency, Schedule: twoAMSchedule, Task: oneDayTaskWindow, Project: projSpec}
	dagSpec = append(dagSpec, specs[spec1])
	specs[spec2] = models.JobSpec{Name: spec2, Dependencies: getDependencyObject(specs, spec1), Schedule: twoAMSchedule, Task: threeDayTaskWindow, Project: projSpec}
	dagSpec = append(dagSpec, specs[spec2])
	specs[spec3] = models.JobSpec{Name: spec3, Dependencies: getDependencyObject(specs, spec2), Schedule: twoAMSchedule, Task: threeDayTaskWindow, Project: projSpec}
	dagSpec = append(dagSpec, specs[spec3])
	specs[spec4] = models.JobSpec{Name: spec4, Dependencies: noDependency, Schedule: hourlySchedule, Task: threeDayTaskWindow, Project: projSpec}
	dagSpec = append(dagSpec, specs[spec4])
	specs[spec5] = models.JobSpec{Name: spec5, Dependencies: getDependencyObject(specs, spec4), Schedule: dailySchedule, Task: threeDayTaskWindow, Project: projSpec}
	dagSpec = append(dagSpec, specs[spec5])
	specs[spec6] = models.JobSpec{Name: spec6, Dependencies: getDependencyObject(specs, spec4, spec5), Schedule: dailySchedule, Task: threeDayTaskWindow, Project: projSpec}
	...
	t.Run("GetStatus", func(t *testing.T) {
		...
		t.Run("should fail if unable to fetch replay spec", func(t *testing.T) {
			...
			_, err := jobSvc.GetStatus(ctx, replayID)
		...
		t.Run("should return error when unable to get run status of a job", func(t *testing.T) {
			...
			replayManager.On("GetRunStatus", ctx, replaySpec, specs[spec1]).Return([]models.JobStatus{}, errors.New(errorMsg))
		...
		t.Run("should return error when unable to get run status of a dependent job", func(t *testing.T) {
			...
			defer replayManager.AssertExpectations(t)
			replayManager.On("GetReplay", replayID).Return(replaySpec, nil)
			replayManager.On("GetRunStatus", ctx, replaySpec, jobSpec0).Return([]models.JobStatus{jobStatusList[0], jobStatusList[1], jobStatusList[2]}, nil)
			errorMsg := "unable to get status of a run"
			replayManager.On("GetRunStatus", ctx, replaySpec, jobSpec1).Return([]models.JobStatus{}, errors.New(errorMsg))
		...
		t.Run("should return error when unable to get run status of a dependent's dependent job", func(t *testing.T) {
			...
			replayManager.On("GetReplay", replayID).Return(replaySpec, nil)
			replayManager.On("GetRunStatus", ctx, replaySpec, jobSpec0).Return([]models.JobStatus{jobStatusList[0], jobStatusList[1], jobStatusList[2]}, nil)
			replayManager.On("GetRunStatus", ctx, replaySpec, jobSpec1).Return([]models.JobStatus{jobStatusList[0], jobStatusList[1], jobStatusList[2]}, nil)
			errorMsg := "unable to get status of a run"
			replayManager.On("GetRunStatus", ctx, replaySpec, jobSpec2).Return([]models.JobStatus{}, errors.New(errorMsg))
