...
func (srv *Service) prepareJobSpecMap(replayRequest models.ReplayRequest) (map[string]models.JobSpec, error) {
	...
	for _, currSpec := range jobSpecs {
		currSpec.Project = replayRequest.Project
...
func (srv *Service) GetStatus(ctx context.Context, reqUUID uuid.UUID) (models.ReplayState, error) {
	// Get replay
	replaySpec, err := srv.replayManager.GetReplay(reqUUID)
	if err != nil {
		return models.ReplayState{}, err
	}

	// updating tree with status per run
	rootInstance, err := srv.prepareReplayStatusTree(ctx, replaySpec)
	if err != nil {
		return models.ReplayState{}, err
	}
...
func (srv *Service) prepareReplayStatusTree(ctx context.Context, replaySpec models.ReplaySpec) (*tree.TreeNode, error) {
	runsWithStatus := set.NewTreeSetWith(TimeOfJobStatusComparator)
	jobStatusList, err := srv.replayManager.GetRunStatus(ctx, replaySpec, replaySpec.Job)
	if err != nil {
		return nil, err
	}
	...
	return srv.populateDownstreamRunsWithStatus(ctx, replaySpec, replaySpec.ExecutionTree)
}

func (srv *Service) populateDownstreamRunsWithStatus(ctx context.Context, replaySpec models.ReplaySpec, parentNode *tree.TreeNode) (*tree.TreeNode, error) {
	for _, dependent := range parentNode.Dependents {
		runsWithStatus := set.NewTreeSetWith(TimeOfJobStatusComparator)
		jobStatusList, err := srv.replayManager.GetRunStatus(ctx, replaySpec, dependent.Data.(models.JobSpec))
		if err != nil {
			return nil, err
		}
		for _, jobStatus := range jobStatusList {
		...
		_, err = srv.populateDownstreamRunsWithStatus(ctx, replaySpec, dependent)
		if err != nil {
			return nil, err
		}
	}
