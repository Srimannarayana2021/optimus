...
func TestReplaySyncer(t *testing.T) {
	...
	t.Run("Sync", func(t *testing.T) {
		t.Run("should not return error when no replay with sync criteria found", func(t *testing.T) {
			...
			replaySyncer := job.NewReplaySyncer(replaySpecRepoFac, nil, time.Now)
		...
		t.Run("should mark state of running replay to success if all instances are success", func(t *testing.T) {
			...
			scheduler.On("GetDagRunStatus", ctx, specs[spec1], startDate, batchEndDate, reqBatchSize).Return(jobStatus, nil).Once()
			scheduler.On("GetDagRunStatus", ctx, specs[spec2], startDate, batchEndDate, reqBatchSize).Return(jobStatus, nil).Once()
			...
			replaySyncer := job.NewReplaySyncer(replaySpecRepoFac, scheduler, time.Now)
			err := replaySyncer.Sync(context.TODO(), runTimeout)
		...
		t.Run("should mark timeout replay as failed", func(t *testing.T) {
			...
			replayRepository := new(mock.ReplayRepository)
			defer replayRepository.AssertExpectations(t)
			replayRepository.On("GetByStatus", job.ReplayStatusToSynced).Return(replaySpec, nil)
		...
		t.Run("should return error when unable to get dag run status from scheduler", func(t *testing.T) {
			...
			scheduler.On("GetDagRunStatus", ctx, specs[spec1], startDate, batchEndDate, reqBatchSize).Return([]models.JobStatus{}, errors.New(errorMsg)).Once()
