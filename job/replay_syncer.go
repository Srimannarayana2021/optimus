...
func (s Syncer) syncRunningReplay(context context.Context, replaySpec models.ReplaySpec, replaySpecRepo store.ReplaySpecRepository) error {
	stateSummary, err := s.checkInstanceState(context, replaySpec)
	if err != nil {
		return err
	}
...
func (s Syncer) checkInstanceState(ctx context.Context, replaySpec models.ReplaySpec) (map[string]int, error) {
	...
	for _, node := range replaySpec.ExecutionTree.GetAllNodes() {
		...
		jobStatusAllRuns, err := s.scheduler.GetDagRunStatus(ctx, node.Data.(models.JobSpec), replaySpec.StartDate, batchEndDate, schedulerBatchSize)
		if err != nil {
			return nil, err
		}
