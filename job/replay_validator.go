...
func (v *Validator) validateRunningInstance(ctx context.Context, reqReplayNodes []*tree.TreeNode, reqInput models.ReplayRequest) error {
	for _, reqReplayNode := range reqReplayNodes {
		...
		reqInput.Job.Project = reqInput.Project
		jobStatusAllRuns, err := v.scheduler.GetDagRunStatus(ctx, reqReplayNode.Data.(models.JobSpec), reqInput.Start, batchEndDate, schedulerBatchSize)
		if err != nil {
			return err
		}
